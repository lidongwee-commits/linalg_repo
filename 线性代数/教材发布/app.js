...
window.addEventListener('DOMContentLoaded',function(){
  if(window.renderMathInElement){
    ...
    // ===== 视口门控 · 分帧渲染（对齐高等数学：根治「漏渲染 + 卡顿」）=====
    // 渲染单元 = 叶子块(p/li/td/...) 与每个含公式的「裸文本节点」(用 span 包裹单独渲染).
    // 关闭的折叠内部单元暂不收集；点开时才插到队首立即渲染，滚动期完全空闲.
...
function initQuickNotes(){
  ...
  var CHAPTERS=[
    {ch:1,name:'行列式'},{ch:2,name:'矩阵及其运算'},{ch:3,name:'向量组'},{ch:4,name:'线性方程组'},{ch:5,name:'相似矩阵与二次型'}
  ];
  function curCh(){
