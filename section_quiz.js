...
(function () {
      ...
      if (st.done === st.total) {
        ...
        if (!st.awarded) {
          ...
          try {
            ...
            // 知识之塔联动：章节测试完成后塔即时刷新并提示
            if (window.TowerGame && window.TowerGame.onChapterTest) {
              window.TowerGame.onChapterTest(st.ch, st.right, st.total);
            }
          } catch (e) {}
        }
      }
      ...
      if (q.type === "mc") {
        ...
        opts.forEach(function (op) {
          op.addEventListener("click", function () {
            ...
            if (ok) {
              fb.innerHTML = '<div class="sq-ans">✅ 正确</div>' + (q.fb ? '<div class="sq-exp">' + esc(q.fb) + '</div>' : "");
            } else {
              fb.innerHTML = '<div class="sq-ans">❌ 正确答案：<b>' + esc(q.ans) + '</b></div>' + (q.fb ? '<div class="sq-exp">' + esc(q.fb) + '</div>' : "");
            }
  ...
  function initCard(card) {
    ...
    card.innerHTML = '<div class="sq-header">' +
      '<span class="sq-title">⚡ 学完就练</span>' +
