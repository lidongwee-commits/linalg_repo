...
window.addEventListener('DOMContentLoaded',function(){
  if(window.renderMathInElement){
    ...
    // ---------- 视口门控 · 分帧渲染（根治「漏渲染 + 卡顿」） ----------
    // 渲染单元 = 叶子块(<p>/<li>/<td>/<th>/<h*>/<blockquote>) 与
    //           每个含公式的「裸文本节点」(用 <span> 包裹后单独渲染).
    // 两处关键修正（前版卡顿/漏公式的真正根因）：
    //   ① hasDelim 之前正则写错，漏匹配 \( \) \[ \] ，导致证明里大量
    //      \[...\] / \(...\) 裸节点永远不被渲染 → 公式缺失.现已修正.
    //   ② 之前「全文档每帧都渲染」：后台持续占满 CPU → 滚动卡顿.
    //      现改为 IntersectionObserver 视口门控：只渲染离视口约 1 屏的单元，
    //      滚到哪渲染到哪，空闲时 CPU 完全让出，滚动顺滑.
    var LEAF_SEL='p,li,td,th,h1,h2,h3,h4,h5,blockquote';
    var IGNORE_TAGS={SCRIPT:1,STYLE:1,NOSCRIPT:1,TEXTAREA:1,PRE:1,CODE:1,SVG:1};
    var hasDelim=function(t){ return /\\\(|\\\)|\\\[|\\\]|\$/.test(t); };
    var Q=[], active=[], draining=false, done=new WeakSet();
    function inClosedFold(el){ return el.closest && el.closest('details:not([open])'); }
    function isLeaf(el){ try{ return el.matches(LEAF_SEL); }catch(e){ return false; } }

    function collectUnits(root, units){
      if(root.nodeType===1){
        var tag=root.tagName;
        if(IGNORE_TAGS[tag]) return;
        if(root.matches && root.matches('details:not([open])')) return;   // 关闭的折叠内部暂不收集
        if(root.classList && (root.classList.contains('katex')||root.classList.contains('katex-lazy'))) return;
      }
      Array.from(root.childNodes).forEach(function(ch){
        if(ch.nodeType===1){
          if(isLeaf(ch)){ units.push(ch); }
          else { collectUnits(ch, units); }
        } else if(ch.nodeType===3){
          if(!/^\s*$/.test(ch.textContent) && hasDelim(ch.textContent)){
            var sp=document.createElement('span');
            root.insertBefore(sp, ch); sp.appendChild(ch);                 // 包成独立单元，单独渲染
            units.push(sp);
          }
        }
      });
    ...
    // 摘要标题里的公式立即渲染（折叠关闭时也可见）
    document.querySelectorAll('summary').forEach(function(s){ try{ renderMathInElement(s,katexOpts); fixVdots(s); }catch(e){} });

    // 收集全部单元；关闭的折叠内部单元先排除（toggle 时再补）
    var ALL=[]; collectUnits(document.body, ALL);
    ALL.forEach(function(u){ if(!inClosedFold(u)) Q.push(u); });

    function renderOne(u){
      if(done.has(u)) return;
      try{ renderMathInElement(u,katexOpts); fixVdots(u); }catch(e){}
      done.add(u);
    }
    var raf=window.requestAnimationFrame||function(cb){ return setTimeout(cb,16); };
    function startDrain(){ if(draining) return; draining=true; raf(drain); }
    function drain(){
      var t0=performance.now();
      while(active.length && (performance.now()-t0)<6){ renderOne(active.shift()); }  // 每帧 ≤6ms，绝不整块同步
      if(active.length) raf(drain); else draining=false;
    }
    function enqueue(u){ if(done.has(u)||active.indexOf(u)>=0) return; active.push(u); startDrain(); }

    // 视口门控：离视口约 1 屏的单元才进入 active 渲染队列
    if('IntersectionObserver' in window){
      var io=new IntersectionObserver(function(entries){
        entries.forEach(function(en){
          if(en.isIntersecting){
            var u=en.target, i=Q.indexOf(u);
            if(i>=0) Q.splice(i,1);
            enqueue(u);
          }
        });
      },{rootMargin:'1000px 0px'});
      Q.forEach(function(u){ io.observe(u); });
    } else {
      ALL.forEach(function(u){ Q.push(u); });
      active=Q.slice(); startDrain();
    }

    // 折叠展开：把内部尚未渲染的单元插到队首（单元小，分帧不卡）
    document.querySelectorAll('details').forEach(function(d){
      d.addEventListener('toggle',function(){
        if(!d.open) return;
        var fu=[]; collectUnits(d, fu);
        fu.forEach(function(u){
          if(!done.has(u)){ var i=Q.indexOf(u); if(i>=0) Q.splice(i,1); active.unshift(u); }
        });
        startDrain();
      });
    });
...
function initQuickNotes(){
  var LS_KEY='gaoshu_notes_v1';
  var BOOK = (LS_KEY === 'gaoshu_notes_v1') ? 'gdsx' : 'linalg';
  // 把笔记并入 study_v1.notes[BOOK]，交由能量柱统一云端同步（与能量/塔同一「同步码」）
  function flushStudy() {
    try {
  ...
  // 云端合并后回填本地笔记（按 ts 去重，不丢本地）
  function refillNotes() {
    try {
      var s = JSON.parse(localStorage.getItem('study_v1')) || {};
      var cloud = (s.notes && s.notes[BOOK]) || [];
      if (!cloud.length) return;
      var local = load(), seen = {};
      local.forEach(function (n) { seen[n.ts] = 1; });
      var changed = false;
      cloud.forEach(function (n) { if (!seen[n.ts]) { local.push(n); seen[n.ts] = 1; changed = true; } });
      if (changed) { save(local); if (panel.classList.contains('open')) render(); }
    } catch (e) {}
  }
  var CHAPTERS=[
    {ch:1,name:'函数与极限'},{ch:2,name:'极限与连续'},{ch:3,name:'导数应用'},
    {ch:4,name:'不定积分'},{ch:5,name:'定积分'},{ch:6,name:'积分应用'},
    {ch:7,name:'微分方程'},{ch:8,name:'向量几何'},{ch:9,name:'多元微分'},
    {ch:10,name:'重积分'},{ch:11,name:'线面积分'},{ch:12,name:'无穷级数'},{ch:13,name:'傅里叶级数'}
  ];
  function curCh(){
    ...
    if(h2){
      var mm=h2.id.match(/^s(\d+)/);
      if(mm) return parseInt(mm[1],10);
    }
    return 1;
  }
  function load(){ try{ return JSON.parse(localStorage.getItem(LS_KEY))||[]; }catch(e){ return []; } }
  function save(notes){ localStorage.setItem(LS_KEY,JSON.stringify(notes)); flushStudy(); }
  function fmtDate(ts){ var d=new Date(ts); return (d.getMonth()+1)+'/'+d.getDate()+' '+d.getHours()+':'+(d.getMinutes()<10?'0':'')+d.getMinutes(); }

  var fab=document.createElement('button');
  fab.className='qn-fab'; fab.title='学习笔记'; fab.innerHTML='&#9998;';
  document.body.appendChild(fab);

  var panel=document.createElement('div');
  panel.className='qn-panel';
  panel.innerHTML=
    '<div class="qn-head"><b>&#9998; 学习笔记</b><span class="qn-close">&times;</span></div>'+
    '<div class="qn-list" id="qn-list"></div>'+
    '<div class="qn-form">'+
      '<textarea id="qn-input" placeholder="记下这节的疑点、公式或易错点..."></textarea>'+
      '<div class="qn-row">'+
        '<select id="qn-ch"></select>'+
        '<button id="qn-add">保存</button>'+
      '</div>'+
    '</div>';
  document.body.appendChild(panel);

  var chSel=document.getElementById('qn-ch');
  CHAPTERS.forEach(function(c){
    var opt=document.createElement('option'); opt.value=c.ch; opt.textContent='第'+c.ch+'章 '+c.name;
    chSel.appendChild(opt);
  });
  chSel.value=String(curCh());

  function render(){
    var notes=load(), list=document.getElementById('qn-list');
    if(notes.length===0){ list.innerHTML='<div class="qn-empty">还没有笔记.<br>遇到薄弱点随时记下来，复习时重点突破.</div>'; return; }
    notes.sort(function(a,b){ return b.ts-a.ts; });
    list.innerHTML=notes.map(function(n,i){
      var chName=(CHAPTERS.find(function(c){ return c.ch===n.ch; })||{name:''}).name;
      return '<div class="qn-item">'+
        '<p>'+escapeHtml(n.text)+'</p>'+
        '<div class="qn-meta">'+
          '<span><span class="qn-tag">第'+n.ch+'章 '+escapeHtml(chName)+'</span> · '+fmtDate(n.ts)+'</span>'+
          '<span class="qn-del" data-i="'+i+'">删除</span>'+
        '</div>'+
      '</div>';
    }).join('');
    list.querySelectorAll('.qn-del').forEach(function(btn){
      btn.addEventListener('click',function(){
        var notes=load(); notes.splice(parseInt(btn.dataset.i,10),1); save(notes); render();
      });
    });
  }
  function escapeHtml(s){ return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }

  document.getElementById('qn-add').addEventListener('click',function(){
    var ta=document.getElementById('qn-input'), text=ta.value.trim();
    if(!text) return;
    var notes=load();
    notes.push({ch:parseInt(chSel.value,10), text:text, ts:Date.now()});
    save(notes); ta.value=''; render();
  });

  fab.addEventListener('click',function(){ panel.classList.toggle('open'); if(panel.classList.contains('open')) render(); });
  panel.querySelector('.qn-close').addEventListener('click',function(){ panel.classList.remove('open'); });
  render();
  // 多平台同步：云端合并完成后回填笔记；加 1.8s 兜底防竞态
  document.addEventListener('study:cloud-synced', function () { refillNotes(); });
  setTimeout(refillNotes, 1800);
}
