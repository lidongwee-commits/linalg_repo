...
(function () {
  ...
  var CH = { 1: "函数与极限", 2: "极限与连续", 3: "微分中值定理与导数应用", 4: "不定积分", 5: "定积分",
    6: "定积分的应用", 7: "微分方程", 8: "向量代数与空间解析几何", 9: "多元函数微分法及其应用",
    10: "重积分", 11: "曲线积分与曲面积分", 12: "无穷级数", 13: "傅里叶级数" };
  ...
  function renderBrowse() {
    ...
    for (var c = 1; c <= 13; c++) html += '<option value="' + c + '">第' + c + '章 · ' + (CH[c] || c) + "</option>";
    html += "</select>";
  ...
  function renderWrong() {
    ...
    if (!w.length) html += '<p class="muted">暂无错题.交卷后答错的客观题会自动收录于此.</p>';
      ...
      html += '<div class="ex-list">' + w.map(function (q) {
        var _d = q.diff != null ? q.diff : ({ mc: 1, fill: 2, calc: 2, app: 3, proof: 3 }[q.type] || 2);
        var card = questionCard({ ch: q.ch, type: q.type, src: q.src, stem: q.stem, options: q.options, ans: q.ans, fb: q.fb, diff: _d, userAns: q.userAns }, true);
        return card;
      }).join("") + "</div>";
    }
    pane.innerHTML = html;
